...
class Kodi:
    def __init__(self, context):
        ...
        self.scheme   = self.GetEnvironmentValue('KODI_SCHEME', 'http')
        self.subpath  = self.GetEnvironmentValue('KODI_SUBPATH', '')
        self.address  = self.GetEnvironmentValue('KODI_ADDRESS', '127.0.0.1')
        self.port     = self.GetEnvironmentValue('KODI_PORT', '8080')
        self.user     = self.GetEnvironmentValue('KODI_USERNAME', 'kodi')
        self.password = self.GetEnvironmentValue('KODI_PASSWORD', 'kodi')
        ...
        self.user = config.get(self.deviceId, 'username')
        self.password = config.get(self.deviceId, 'password')

    def GetEnvironmentValue(self, name, default):
    ...
    def SendCommand(self, command):
      ...
      print "Sending request to %s as %s/%s from device %s" % (url, self.user, self.password, self.deviceId)

      try:
        r = requests.post(url, data=command, auth=(self.user, self.password))
      except:
        return {}
