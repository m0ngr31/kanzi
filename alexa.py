    ...
    if show:
        ...
        if num_of_unwatched > 0:
            if num_of_unwatched == 1:
                ...
                response_text = render_template('multiple_unseen_show', show_name=heard_show,
                                                num=num_of_unwatched).encode("utf-8")
        else:
...
def find_and_play(needle, content=['video', 'audio'], slot_hint='unknown', slot_ignore=""):
    ...
    # Video playlist?
    if 'video' in content and slot_ignore != 'VideoPlaylist' and (
            slot_hint == 'unknown' or slot_hint == 'VideoPlaylist'):
    ...
    # Audio playlist?
    if 'audio' in content and slot_ignore != 'AudioPlaylist' and (
            slot_hint == 'unknown' or slot_hint == 'AudioPlaylist'):
    ...
    if 'video' in content and slot_ignore != 'Show' and (slot_hint == 'unknown' or slot_hint == 'Show'):
        ...
        if show:
            ...
            if episode_id:
                ...
                return render_template('playing_episode_number', season=episode_details['season'],
                                       episode=episode_details['episode'], show_name=needle).encode("utf-8")
...
def alexa_listen_artist(Artist):
    ...
    artist = kodi.FindArtist(heard_artist)
    if artist:
        songs_result = kodi.GetArtistSongs(artist)
        songs = songs_result['result']['songs']
...
def alexa_listen_album(Album, Artist):
    ...
    if Artist:
        ...
        if artist:
            ...
            if 'result' in albums and 'albums' in albums['result']:
                ...
                if album_located:
                    ...
                    kodi.StartAudioPlaylist()
                    response_text = render_template('playing_album_artist', album_name=heard_album,
                                                    artist=heard_artist).encode("utf-8")
                else:
                    response_text = render_template('could_not_find_album_artist', album_name=heard_album,
                                                    artist=heard_artist).encode("utf-8")
            else:
                response_text = render_template('could_not_find_album_artist', album_name=heard_album,
                                                artist=heard_artist).encode("utf-8")
        ...
        album = kodi.FindAlbum(heard_album)
        if album:
            kodi.PlayerStop()
            kodi.ClearAudioPlaylist()
            kodi.AddAlbumToPlaylist(album)
        ...
        if artist:
            ...
            if 'result' in songs and 'songs' in songs['result']:
                ...
                if song_located:
                    ...
                    kodi.StartAudioPlaylist()
                    response_text = render_template('playing_song_artist', song_name=heard_song,
                                                    artist=heard_artist).encode("utf-8")
                else:
                    response_text = render_template('could_not_find_song_artist', song_name=heard_song,
                                                    artist=heard_artist).encode("utf-8")
            else:
                response_text = render_template('could_not_find_song_artist', song_name=heard_song,
                                                artist=heard_artist).encode("utf-8")
    ...
    if artist:
        ...
        if 'result' in songs and 'songs' in songs['result']:
            ...
            if song_located:
                ...
                kodi.AddSongToPlaylist(song_result)
                kodi.StartAudioPlaylist()
                response_text = render_template('playing_song_artist', song_name=heard_search,
                                                artist=heard_artist).encode("utf-8")
            else:
                albums = kodi.GetArtistAlbums(artist)
                if 'result' in albums and 'albums' in albums['result']:
                    ...
                    if album_located:
                        ...
                        kodi.AddAlbumToPlaylist(album_result)
                        kodi.StartAudioPlaylist()
                        response_text = render_template('playing_album_artist', album_name=heard_search,
                                                        artist=heard_artist).encode("utf-8")
                    else:
                        response_text = render_template('could_not_find_multi', heard_name=heard_search,
                                                        artist=heard_artist).encode("utf-8")
                else:
                    response_text = render_template('could_not_find_multi', heard_name=heard_search,
                                                    artist=heard_artist).encode("utf-8")
...
def alexa_watch_random_movie(Genre):
    ...
    if len(movies_array):
        ...
        if genre_located:
            response_text = render_template('playing_genre', genre=genre_located['label'],
                                            movie_name=random_movie['label']).encode("utf-8")
        else:
...
def alexa_watch_movie(Movie):
    ...
    if (len(heard_movie) > 0):
        movie = kodi.FindMovie(heard_movie)
        if movie:
            if kodi.GetMovieDetails(movie)['resume']['position'] > 0:
                action = render_template('resuming_empty').encode("utf-8")
            else:
...
def alexa_watch_random_episode(Show):
    ...
    if show:
        ...
        response_text = render_template('playing_episode_number', season=episode_details['season'],
                                        episode=episode_details['episode'], show_name=heard_show).encode("utf-8")
    else:
...
def alexa_watch_episode(Show, Season, Episode):
    ...
    if show:
        ...
        if episode_id:
            ...
            response_text = render_template('playing_action_episode_number', action=action, season=heard_season,
                                            episode=heard_episode, show_name=heard_show).encode("utf-8")

        else:
            response_text = render_template('could_not_find_episode_show', season=heard_season, episode=heard_episode,
                                            show_name=heard_show).encode("utf-8")
    else:
        response_text = render_template('could_not_find_show', heard_show=heard_show).encode("utf-8")
...
def alexa_watch_next_episode(Show):
    ...
    if show:
        ...
        if next_episode_id:
            ...
            response_text = render_template('playing_action_episode_number', action=action,
                                            season=episode_details['season'], episode=episode_details['episode'],
                                            show_name=heard_show).encode("utf-8")
        else:
...
def alexa_watch_last_show():
    ...
    try:
        ...
        if next_episode_id:
            ...
            response_text = render_template('playing_action_episode_number', action=action,
                                            season=episode_details['season'], episode=episode_details['episode'],
                                            show_name=last_show_obj['result']['episodes'][0]['showtitle']).encode(
                "utf-8")
        else:
            response_text = render_template('no_new_episodes_show',
                                            show_name=last_show_obj['result']['episodes'][0]['showtitle']).encode(
                "utf-8")
    except:
        response_text = render_template('error_parsing_results').encode("utf-8")
...
def alexa_what_new_episodes():
    ...
    if num_shows == 0:
        ...
        if count == 1:
            ...
            response_text = render_template('multiple_new_episodes', show_name=new_show_names[0], count=count).encode(
                "utf-8")
    else:
